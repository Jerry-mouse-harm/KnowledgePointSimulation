...
实现带通滤波器
"""
class BandPassFilter(BasicFilter):
    def __init__(self, sym_exper , sample_frequency, T, range_size):
    ...
    def create_interactive_plot(self, cutoff_range=None):
        ...
        # 生成多个截止频率值用于slider
        num_steps = 50
        cutoff_freqs = np.linspace(min_cutoff, max_cutoff - self.range_size, num_steps)
        ...
        fig.update_layout(
            title=dict(
                text=f'交互式带通滤波器 - 信号: x(t) = {sp.pretty(self.sym_exper)}',
                x=0.5,
                xanchor='center'
            ),
            sliders=sliders,
...
if __name__ == '__main__':
    # 示例1: 使用默认信号 sin(t)
    print("示例1: 带通滤波 sin(t)")
    filter1 = BandPassFilter(sin(t),  100, 4, 5)
